...
os.environ["CUDA_VISIBLE_DEVICES"] = "1,2"  # 指定第一块gpu
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # 去除假阳性，判断连续5个帧是否均为阳性  frame是当前帧数 seen计算总数的
            num_bbx.append(len(det))
            if seen > 5:
                sum_bbox4 = sum(num_bbx[seen-2-4:seen-2])  # 5帧一组
                if sum_bbox4 == 0:       # 4帧全空
                    im0 = im             # 输出原图
                    FLAG = 0             # 跳过画框
            ...
            if len(det):
                ...
                xy.append([0,0])
            if seen > 1:       # dis = (x2-x1)^2+(y2-y1)^2
                dis = (xy[seen-1][0] - xy[seen-2][0])**2 + (xy[seen-1][1] - xy[seen-2][1])**2
                distance.append(dis)
                mean_dis = 5256.35       # 计算的整个视频的连续两帧均值(非零)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp2/weights/best.pt', help='model path(s)') # 修改
    parser.add_argument('--source', type=str, default=ROOT / 'data/images', help='file/dir/URL/glob, 0 for webcam') # 修改
    ...
    parser.add_argument('--project', default='/data/dataset/lhq/test3/runs/detect', help='save results to project/name')
